...
class IntegrationTreeSpec extends FlatSpec with Matchers {
	...
	it should "go to level 12" in {
		Tree(whist).enumeratePlays(12)(_ => false, _ => false).depthFirstTraverse.size shouldBe 93339
	}

	ignore should "go to level 16" in {
		Tree(whist).enumeratePlays(16)(_ => false, _ => false).depthFirstTraverse.size shouldBe 1214963
	}
