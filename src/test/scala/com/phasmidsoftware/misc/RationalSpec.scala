...
import com.phasmidsoftware.misc.Rational.RationalHelper
...
/**
  * @author scalaprof
  */
class RationalSpec extends AnyFlatSpec with should.Matchers with PrivateMethodTester {
  ...
  ignore should "fail to convert to BigInt" in {
    val r = Rational(-1, 0)
  ...
  it should "be decimal when exact" in {
    val r = Rational(1, 2)
    r.render(Some("%5.2")) shouldBe "0.5"
  }
  ...
  ignore should "work for Int.MaxValue" in {
  ...
  ignore should "not work for Int.MaxValue+1" in {
  ...
  ignore should "work for Int.MaxValue+1" in {
  ...
  ignore should "work for inf,inf" in {
    val decorateCompare = PrivateMethod[Int](Symbol("compare"))
