...
case class Rational(n: BigInt, d: BigInt) {
  ...
  /**
    *
    * @param format of the form "%m.nf" will be used only if the Rational appears to be an irrational* number.
    *               * actually, rational numbers with large denominators are also shown this way.
    * @return an Optional String
    */
  def render(format: Option[String] = None): String =
    if (isNaN) "NaN"
    ...
    //    {
    //      val x = toDouble
    //      format map (w => f"$x$w") getOrElse x.toString
    //    }
    else toRationalString

  override def toString: String = render(None)
}
