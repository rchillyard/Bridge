...
class RecapParser extends JavaTokenParsers {
  ...
  def pair: Parser[Pair] = (wholeNumber <~ spacer) ~ opt(direction) ~ playerPlayer ^^
  ...
  def endOfLine: Parser[String] = s"""$newline""".r | """\n""".r | """\r\n""".r | failure("not a proper line-end") // XXX is this OK?

  private def spacer: Parser[String] = """[ \t]*""".r | failure("not a spacer--should be space or tab")
  ...
  private def direction = "E" | "N" | failure("direction must be E or N")
